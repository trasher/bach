...
class PMBFileFormat extends FileFormat
{
    ...
    protected function parseAuthors($data)
    {
        ...
        $has_changed = false;
        //$result = converter("070");
            //echo $result;
            //echo $entry['value'];
            //result = new PMBAuthor($type, $prenom, $nom, $function, $pmb);
            //    $this->addAuthor($result);
    }
    /**
     * Parse converter function
     *
     * @param string $code code fuunction
     *
     * @return string
     */
    protected function converter($code){
        $value=null;
        switch ($code) {
            case "005" :
                $value="Acteur";
